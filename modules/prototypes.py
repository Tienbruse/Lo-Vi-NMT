...
# --- Sửa: bỏ class Embedder cũ, nếu cần thì tạo class nhỏ gọn xuống dưới

class EncoderLayer(nn.Module):
    def __init__(self, d_model, heads, dropout=0.1):
        super().__init__()
        # === REPLACED: dùng PreNorm (Pre-LN) thay cho Norm (Post-LN) ===
        self.pre_norm1 = layers.PreNorm(d_model)        # was layers.Norm(d_model)
        self.pre_norm2 = layers.PreNorm(d_model)        # was layers.Norm(d_model)
        self.attn      = layers.MultiHeadAttention(heads, d_model, dropout=dropout)
        self.ff        = layers.FeedForward(d_model, dropout=dropout)
        self.dropout_1 = nn.Dropout(dropout)
        self.dropout_2 = nn.Dropout(dropout)
    ...
    def forward(self, x, src_mask):
        ...
        x2, sa = self.attn(self.pre_norm1(x),  # === MODIFIED: apply PreNorm to x
                           self.pre_norm1(x),
                           self.pre_norm1(x),
                           src_mask)
        x = x + self.dropout_1(x2)

        # Pre-Norm trước feed-forward
        x2 = self.pre_norm2(x)                 # === MODIFIED: apply PreNorm to x
        x  = x + self.dropout_2(self.ff(x2))
        return x, sa
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, heads, dropout=0.1):
        super().__init__()
        # === REPLACED: PreNorm thay cho Norm ===
        self.pre_norm1 = layers.PreNorm(d_model)  # was layers.Norm(d_model)
        self.pre_norm2 = layers.PreNorm(d_model)  # was layers.Norm(d_model)
        self.pre_norm3 = layers.PreNorm(d_model)  # was layers.Norm(d_model)
    ...
    def forward(self, x, memory, src_mask, trg_mask):
        # 1) Self-attention với PreNorm
        x_sa, sa = self.attn_1(self.pre_norm1(x),  # === MODIFIED ===
                               self.pre_norm1(x),
                               self.pre_norm1(x),
                               trg_mask)
        x = x + self.dropout_1(x_sa)

        # 2) Encoder–Decoder attention với PreNorm
        x_na, na = self.attn_2(self.pre_norm2(x),  # === MODIFIED ===
                               memory,
                               memory,
                               src_mask)
        x = x + self.dropout_2(x_na)

        # 3) Feed-forward với PreNorm
        x2 = self.pre_norm3(x)                    # === MODIFIED ===
        x  = x + self.dropout_3(self.ff(x2))
        return x, (sa, na)
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, d_model, N, heads, dropout, max_seq_length=200):
        ...
        # === REMOVED: không dùng PositionalEncoder nữa ===
        # self.pe = layers.PositionalEncoder(d_model, dropout=dropout, max_seq_length=max_seq_length)

        self.layers = get_clones(EncoderLayer(d_model, heads, dropout), N)
        # === REPLACED: PreNorm cuối cùng ===
        self.pre_norm = layers.PreNorm(d_model)  # was layers.Norm(d_model)

        self._max_seq_length = max_seq_length

    def forward(self, src, src_mask, output_attention=False, seq_length_check=False):
        if seq_length_check and src.size(1) > self._max_seq_length:
            src = src[:, :self._max_seq_length]
            src_mask = src_mask[:, :, :self._max_seq_length]
        ...
        x = self.pre_norm(x)  # === MODIFIED: cuối cùng apply PreNorm
        return (x, attentions) if output_attention else x
